...
class SAPConfig:
    ...
    def _validate_credentials(self):
        ...
        if missing_fields:
            raise SecurityError(
                f"Missing required SAP credentials: {', '.join(missing_fields)}"
            )
    ...
    @classmethod
    def from_dotenv(
        cls, env_file: Optional[Path] = None, prefix: str = "SAP_"
    ) -> "SAPConfig":
        """Create config from .env file (recommended for development)."""
        if not DOTENV_AVAILABLE:
            raise SecurityError(
                "python-dotenv package is required for .env file support. Install with: pip install python-dotenv"
            )
    ...
    @classmethod
    def from_keyring(
        cls, service_name: str = "AgentSAP", username: str = "sap_connection"
    ) -> "SAPConfig":
        """Create config from system keyring (recommended for production)."""
        if not KEYRING_AVAILABLE:
            raise SecurityError(
                "keyring package is required for keyring support. Install with: pip install keyring"
            )
        ...
        try:
            ...
            if not encrypted_data:
                raise SecurityError(
                    f"No credentials found in keyring for service '{service_name}', user '{username}'"
                )
    ...
    def save_to_keyring(
        self, service_name: str = "AgentSAP", username: str = "sap_connection"
    ) -> None:
        ...
        print(f"[OK] Credentials saved to keyring: {service_name}/{username}")
    ...
    def save_to_encrypted_file(self, file_path: Path, password: str) -> None:
        ...
        print(f"[OK] Credentials encrypted and saved to: {file_path}")
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"SAPConfig(user='{self.user}', ashost='{self.ashost}', "
...
class SAPConfigManager:
    ...
    def get_config(method: str = "auto", **kwargs) -> SAPConfig:
        ...
        if method == "auto":
            ...
            return SAPConfig.from_dotenv(
                kwargs.get("env_file"), kwargs.get("prefix", "SAP_")
            )
        elif method == "keyring":
            ...
            return SAPConfig.from_encrypted_file(
                kwargs["file_path"], kwargs["password"]
            )
        else:
    ...
    def setup_credentials_interactive() -> SAPConfig:
        ...
        print("[CONFIG] SAP Credentials Setup")
        print("=" * 30)
        ...
        passwd = input(
            "SAP Password: "
        ).strip()  # Note: visible input, consider getpass
        ...
        print("\n[EMOJI] Choose credential storage method:")
        ...
        if choice == "1" and KEYRING_AVAILABLE:
            ...
            print("[WARNING] Invalid choice or missing dependencies")
...
[EMOJI] SAP Credential Security Recommendations:
